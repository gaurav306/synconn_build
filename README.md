...
```
here `mprs_setpoints`, `mprs_hvac_mode`, `prbs_win_multiple_multistep`, `noise_for_setpoints` are the four signals generated by **Signal_Generator.py**
...
- `signal_generator_details`:
  - `if_plot`: Set to `0` to disable plotting.
  - `frequency_categories`: Define categories of signal frequencies, which can be used for different signals.

- `mprs_setpoints`:
  - `normal_or_uniform_distribution`: Specify the type of distribution for the signal generator.
